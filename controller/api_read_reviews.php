...
try {
    ...
    // Get community reviews by author name when the reviewer is not a registered user
    if (isset($_GET['author'])) {
        $author = trim($_GET['author']);
        $stmt = $cnx->prepare("SELECT * FROM reviews WHERE author = ? AND user_id IS NULL ORDER BY date DESC");
        $stmt->execute([$author]);
        $reviews = $stmt->fetchAll(PDO::FETCH_ASSOC);
