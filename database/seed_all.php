...
try {
    ...
    $cnx->exec("TRUNCATE TABLE comments;");
    $cnx->exec("TRUNCATE TABLE reviews;");
    $cnx->exec("TRUNCATE TABLE favorites;");
    $cnx->exec("TRUNCATE TABLE wishlist;");
    $cnx->exec("TRUNCATE TABLE restaurants;");
    $cnx->exec("TRUNCATE TABLE users;");
    $cnx->exec("SET FOREIGN_KEY_CHECKS = 1;");

    // 2. SEED USERS
    echo "Creating users...\n";
    $usersData = [
        ['nom' => 'Sarah Ben Amor', 'email' => 'sarah@example.com', 'pwd' => '123456'],
        ['nom' => 'Mehdi Trabelsi', 'email' => 'mehdi@example.com', 'pwd' => '123456'],
        ['nom' => 'Emna Gharbi', 'email' => 'emna@example.com', 'pwd' => '123456'],
        ['nom' => 'Yassin Mansour', 'email' => 'yassin@example.com', 'pwd' => '123456'],
        ['nom' => 'Linda Karoui', 'email' => 'linda@example.com', 'pwd' => '123456'],
        ['nom' => 'Ahmed Mejri', 'email' => 'ahmed@example.com', 'pwd' => '123456'],
        ['nom' => 'Sonia Dridi', 'email' => 'sonia@example.com', 'pwd' => '123456'],
        ['nom' => 'Khaled Ben Salem', 'email' => 'khaled@example.com', 'pwd' => '123456'],
        ['nom' => 'Ines Khemiri', 'email' => 'ines@example.com', 'pwd' => '123456'],
        ['nom' => 'Zied Hammami', 'email' => 'zied@example.com', 'pwd' => '123456'],
    ];

    $stmtUser = $cnx->prepare("INSERT INTO users (nom, email, password) VALUES (?, ?, ?)");
    $userIds = [];
    foreach ($usersData as $u) {
        $stmtUser->execute([$u['nom'], $u['email'], hashPwd($u['pwd'])]);
        $userIds[] = [
            'id' => $cnx->lastInsertId(),
            'name' => $u['nom']
        ];
    ...
    foreach ($restaurants as $r) {
        $stmtResto->execute([ $r['name'], $r['cuisine'], $r['category'], $r['address'], $r['city'], $r['phone'], $r['priceRange'], $r['lat'], $r['lng'], $r['tags'], $r['image'], $r['description'], $r['openHours'] ]);
        $restoIds[] = $cnx->lastInsertId();
    }

    // 4. SEED REVIEWS
    echo "Creating reviews...\n";
    $reviewTexts = [
        5 => ["Incredible flavors!", "Best meal ever.", "Perfect service.", "A gem in the city.", "Highly recommended!"],
        4 => ["Very good food.", "Lovely atmosphere.", "Attentive staff.", "Solid choice.", "Great value."],
    ...
    $commentTexts = [
        ...
        "That sounds delicious, need to try it.",
        "Great review, very helpful.",
        "I found it a bit overpriced though.",
        "Did you try their desserts?",
        "Wait time was short for me."
    ];

    $stmtComment = $cnx->prepare("INSERT INTO comments (review_id, user_id, author, text) VALUES (?, ?, ?, ?)");
    foreach ($reviewIds as $rv) {
        ...
        for ($i = 0; $i < $numComments; $i++) {
            ...
            $stmtComment->execute([$rv['id'], $user['id'], $user['name'], $text]);
        }
    }

    echo "SUCCESS! Database fully seeded.\n";
    echo "- Users: " . count($userIds) . "\n";
    echo "- Restaurants: " . count($restoIds) . "\n";
    echo "- Reviews: " . count($reviewIds) . "\n";
